...
static int handle_get_rotId(const overlay_handle_t overlay) {
	return static_cast<const struct handle_t *>(overlay)->rot_ssid;
}
...
public:
	overlay_object(int fb, int rotator, int rot_ssid, int w, int h, int format, int size,
...
// ****************************************************************************
	...
	static overlay_t* overlay_createOverlay(struct overlay_control_device_t *dev,
		...
		if (overlay_start_rotator(rotator, &ctx->rotInfo, w, h, hw_format)) {
	...
	static int overlay_setRot(int rotator, struct msm_rotator_img_info *ri, int flag) {
		...
		/* for now, stop here to return to the same session id */
		ioctl(rotator, MSM_ROTATOR_IOCTL_FINISH, &ri->session_id);
